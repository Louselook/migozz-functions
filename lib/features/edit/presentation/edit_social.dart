...
class _EditSocialScreenState extends State<EditSocialScreen> {
                  ...
                  Expanded(
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final selected = selectedSocials.contains(
                          label.toLowerCase(),
                        );
  ...
  Future<void> saveSocials() async {
    try {
      ...
      List<Map<String, Map<String, dynamic>>> finalEcosystem = List.from(
        updatedEcosystem,
      );

      if (removedSocials.isNotEmpty) {
        // filtramos cualquier posible resto
        finalEcosystem = updatedEcosystem.where((item) {
      ...
      ScaffoldMessenger.of(
        // ignore: use_build_context_synchronously
        context,
      ).showSnackBar(const SnackBar(content: Text('Error saving socials')));
    }
  }
}
